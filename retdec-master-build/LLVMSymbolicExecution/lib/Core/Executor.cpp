...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
	...
	this->solver = new TimingSolver(coreSolver, EqualitySubstitution);
...
void Executor::initSingleModule(llvm::Module* moduleptr)
{
	klee::Interpreter::ModuleOptions opts;
	opts.CheckDivZero = true;
	opts.CheckOvershift = true;
	kmodule = std::unique_ptr<KModule>(new KModule());
	kmodule->module = std::unique_ptr<llvm::Module>(moduleptr);
	kmodule->targetData = std::unique_ptr<llvm::DataLayout>(new DataLayout(moduleptr));
...
MemoryObject * Executor::initializePreAllocedGlobalObjects(ExecutionState &state, ObjectPair *op, const GlobalVariable &v, bool negative)
{
	...
	MemoryObject *mo1 = memory->allocate(fixedStackSize, /*isLocal=*/false,
		/*isGlobal=*/true, /*allocSite=*/&v,
...
void Executor::initializeGlobalObjects(ExecutionState &state) {
	...
	for (const GlobalVariable &v : m->globals()) {
		...
		if (v.isDeclaration() && mo->size) {
			...
			if (varname == "__dso_handle") {
				addr = &__dso_handle; // wtf ?
			}else if(varname.contains("Class_vtable"))
			{
				continue;
			}
			else {
				addr = externalDispatcher->resolveSymbol(varname.str());
			}
			if (!addr) {
				/*klee_error("Unable to load symbol(%.*s) while initializing globals",
					static_cast<int>(varname.size()), varname.data());*/
				/*klee_warning("Unable to load symbol(%s) while initializing globals,try skip",
					varname.data());*/
				continue;
			}
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
	...
	if (f && f->isDeclaration()) {
		...
		std::size_t callingArgs = arguments.size();
		...
		if (!f->isVarArg()) {
			...
			uint64_t* offsets = new uint64_t[callingArgs]; // offsets of variadic arguments
			...
			const CallSite cs(i);
#endif
			for (std::size_t k = funcArgs; k < callingArgs; k++) {
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
	...
	default:
		//	ki->inst->dump();
			//klee_warning("illegal instruction try skip");
...
void Executor::computeOffsetsSeqTy(KGEPInstruction *kgepi,
	ref<klee::ConstantExpr> &constantOffset,
	...
	if (const Constant *c = dyn_cast<Constant>(operand)) {
		ref<klee::ConstantExpr> index =
			evalConstant(c)->SExt(Context::get().getPointerWidth());
		ref<klee::ConstantExpr> addend = index->Mul(
			klee::ConstantExpr::alloc(elementSize, Context::get().getPointerWidth()));
...
void Executor::computeOffsets(KGEPInstruction *kgepi, TypeIt ib, TypeIt ie) {
	ref<klee::ConstantExpr> constantOffset =
		klee::ConstantExpr::alloc(0, Context::get().getPointerWidth());
	uint64_t index = 1;
	for (TypeIt ii = ib; ii != ie; ++ii) {
		if (StructType *st = dyn_cast<StructType>(*ii)) {
			...
			constantOffset = constantOffset->Add(klee::ConstantExpr::alloc(addend,
...
bool Executor::checkMemoryUsage() {
	...
	const std::size_t numStates = states.size();
	const std::size_t calc = numStates - numStates * MaxMemory / totalUsage;
	const std::size_t isone = 1;
	const std::size_t toKill = std::max(isone, calc);
...
void Executor::terminateStateOnError(ExecutionState &state,
	...
	if (EmitAllErrors ||
		...
		if (!ii.file.empty()) {
			klee_warning("ERROR: %s:%d: %s", ii.file.c_str(), ii.line, message.c_str());
		}
		else {
			klee_warning("ERROR: (location information missing) %s", message.c_str());
		}
		if (!EmitAllErrors)
			klee_warning("NOTE: now ignoring this error at this location");
...
void Executor::callExternalFunction(ExecutionState &state,
	...
	Function *function,
	std::vector< ref<Expr> > &arguments) {
	if (function->getName() == "__asm_rep_stosd_memset")
	{
		return;
	}
	if (function->getName() == "__std_exception_destroy")
	{
		terminateStateOnUserError(state, "calling external : unhandled  function :=> __std_exception_destroy");
		return;
	}
...
void Executor::executeMemoryOperation(ExecutionState &state,
	...
	if (success) {
		...
		bool inBounds;
		solver->setTimeout(coreSolverTimeout);
		bool success1 = solver->mustBeTrue(state.constraints, check, inBounds,
			state.queryMetaData);
		solver->setTimeout(time::Span());
		if (!success1) {
			state.pc = state.prevPC;
	...
	/*klee_message("ERROR: (memory error)");
	return;*/
