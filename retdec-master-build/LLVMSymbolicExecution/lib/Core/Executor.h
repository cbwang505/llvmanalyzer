...
private:
  std::size_t fixedStackSize = 0x1000;
  ...
  MemoryObject * initializePreAllocedGlobalObjects(ExecutionState& state, ObjectPair* op, const llvm::GlobalVariable& v,  bool negative);
  ...
  bool isStackPointer(const llvm::Value* v);
  bool isThisPointer(const llvm::Value* v);
...
public:
  Executor(llvm::LLVMContext &ctx, const InterpreterOptions &opts,
   ...
   void initSingleModule(llvm::Module* moduleptr) override;
  virtual ~Executor();
