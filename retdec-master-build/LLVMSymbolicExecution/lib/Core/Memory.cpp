...
void ObjectState::flushRangeForRead(unsigned rangeBase,
  ...
  for (uint64_t offset = rangeBase; offset < rangeBase + rangeSize; offset++) {
...
bool ObjectState::isByteConcrete(uint64_t offset) const {
  return !concreteMask || concreteMask->get(offset);
}

bool ObjectState::isByteUnflushed(uint64_t offset) const {
  return !unflushedMask || unflushedMask->get(offset);
}

bool ObjectState::isByteKnownSymbolic(uint64_t offset) const {
  return knownSymbolics && knownSymbolics[offset].get();
}

void ObjectState::markByteConcrete(uint64_t offset) {
  if (concreteMask)
    concreteMask->set(offset);
}

void ObjectState::markByteSymbolic(uint64_t offset) {
  if (!concreteMask)
    concreteMask = new BitArray(size, true);
  concreteMask->unset(offset);
}

void ObjectState::markByteUnflushed(uint64_t offset) {
  if (unflushedMask)
    unflushedMask->set(offset);
}

void ObjectState::markByteFlushed(uint64_t offset) {
  if (!unflushedMask) {
...
void ObjectState::setKnownSymbolic(uint64_t offset, 
...
ref<Expr> ObjectState::read8(uint64_t offset) const {
  if (isByteConcrete(offset)) {
...
void ObjectState::write8(uint64_t offset, uint8_t value) {
...
void ObjectState::write8(uint64_t offset, ref<Expr> value) {
...
ref<Expr> ObjectState::read(uint64_t offset, Expr::Width width) const {
  // Treat bool specially, it is the only non-byte sized write we allow.
  if (width == Expr::Bool)
    return ExtractExpr::create(read8(offset), 0, Expr::Bool);
...
void ObjectState::write(uint64_t offset, ref<Expr> value) {
...
void ObjectState::write16(uint64_t offset, uint16_t value) {
  unsigned NumBytes = 2;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
...
void ObjectState::write32(uint64_t offset, uint32_t value) {
  unsigned NumBytes = 4;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
    write8(offset + idx, (uint8_t) (value >> (8 * i)));
...
void ObjectState::write64(uint64_t offset, uint64_t value) {
  unsigned NumBytes = 8;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
    write8(offset + idx, (uint8_t) (value >> (8 * i)));
