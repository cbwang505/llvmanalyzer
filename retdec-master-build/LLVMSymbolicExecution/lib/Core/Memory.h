...
public:
  ...
  ref<Expr> read(uint64_t offset, Expr::Width width) const;
  ref<Expr> read8(uint64_t offset) const;

  void write(uint64_t offset, ref<Expr> value);
...
private:
  ...
  void write8(uint64_t offset, ref<Expr> value);
  ...
  bool isByteConcrete(uint64_t offset) const;

  /// isByteKnownSymbolic ==> !isByteConcrete
  bool isByteKnownSymbolic(uint64_t offset) const;

  /// isByteUnflushed(i) => (isByteConcrete(i) || isByteKnownSymbolic(i))
  bool isByteUnflushed(uint64_t offset) const;

  void markByteConcrete(uint64_t offset);
  void markByteSymbolic(uint64_t offset);
  void markByteFlushed(uint64_t offset);
  void markByteUnflushed(uint64_t offset);
  void setKnownSymbolic(uint64_t offset, Expr *value);
