	...
	if (DeterministicAllocation) {
		...
		/*char *newSpace =
			(char *)mmap(expectedAddress, spaceSize, PROT_READ | PROT_WRITE,
						 ...
						 */
		char *newSpace = (char *)malloc(spaceSize);
		if (newSpace == nullptr) {
			klee_error("Couldn't mmap() memory for deterministic allocations");
		}
		//‘ –Ì…Í«Î‘§∑÷≈‰ƒ⁄¥Ê
		DeterministicStartAddress.setValue(reinterpret_cast<uint64_t>(newSpace));
...
MemoryManager::~MemoryManager() {
	while (!objects.empty()) {
		...
		{
			uint64_t addressReal = findRealAddress(mo->address);
			if (addressReal)
			{
				free((void *)addressReal);
			}
		}			
		objects.erase(mo);
		delete mo;
...
MemoryObject *MemoryManager::allocate(uint64_t size, bool isLocal,
	...
	++stats::allocations;
	uint64_t addressfinal = bits64::truncateToNBits(address, Context::get().getPointerWidth());
	addr2real.emplace(addressfinal, address);
	MemoryObject *res = new MemoryObject(addressfinal, size, isLocal, isGlobal, false,
		allocSite, this);
	objects.insert(res);
	return res;
}
uint64_t MemoryManager::findRealAddress(uint64_t addressfinal)
{
	auto kv = addr2real.find(addressfinal);
	if (kv != addr2real.end())
	{
		return kv->second;
	}
