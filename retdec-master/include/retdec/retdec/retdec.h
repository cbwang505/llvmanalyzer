...
bool decompile_ida(
	retdec::config::Config& config,
	std::tuple<std::unique_ptr<retdec::utils::io::Logger>&, std::unique_ptr<retdec::utils::io::Logger>&, std::unique_ptr<retdec::utils::io::Logger>&> tp,
	uint64_t segaddr = 0,
	std::string* outString = nullptr, std::vector<std::uint8_t>* outBuff = nullptr);
