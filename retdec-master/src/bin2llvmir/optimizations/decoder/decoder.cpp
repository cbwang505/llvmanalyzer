...
namespace retdec {
	namespace bin2llvmir {
		...
		bool Decoder::validRange(common::Address start)
		{
		...
		void Decoder::decodeJumpTarget(const JumpTarget& jt)
		{
			...
			if (bb == nullptr)
			{
				...
				{
					AddressRange sr(start, start + bytes.second);
					_ranges.remove(sr);
					return;
				}
			}
			...
			do
			{
				...
				bbEnd |= getJumpTargetsFromInstruction(start,oldAddr, res, bytes.second);
		...
		bool Decoder::addJumpTargets(common::Address t, JumpTarget::eType tp, common::Address addr)
		{
			_jumpTargets.push(
				t,
				tp,
