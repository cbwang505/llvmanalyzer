...
class ProgramOptions
{
	public:
		...
		void dump()
		{
			...
			Log::info() << std::endl;
			ss << "Program Options:" << std::endl;
			ss << "\t" << "arch   : " << arch << " (" << _arch << ")" << std::endl;
			ss << "\t" << "base   : " << std::hex << base << " (" << _base << ")" << std::endl;
			ss << "\t" << "code   : " << tmp << " (" << _code << ")" << std::endl;
			ss << "\t" << "asm text : " << text << std::endl;
			ss << "\t" << "b mode : " << std::hex << basicMode << " (" << _basicMode << ")" << std::endl;
			ss << "\t" << "e mode : " << std::hex << extraMode << " (" << _extraMode << ")" << std::endl;
			ss << "\t" << "out    : " << outFile << std::endl;			
...
void printVersion()
{
	...
	Log::info() << std::endl;
	Log::info() << "Capstone version: " << std::to_string(version) << " (major: " << std::to_string(major)
			<< ", minor: " << std::to_string(minor) << ")" << std::endl;
}
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		int count = 0;
		auto f1 = res.insns.begin()->first->getFunction();
		...
		std::ostringstream ss;
		ss << "ALL instruction count :=> " << count;
		Log::info() << ss.str()<<std::endl;
