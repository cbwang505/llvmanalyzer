 ...
 * \return 
 */
uint64_t HLLWriter::DetermineTypeSize(ShPtr<Type> elemType)
{
	uint64_t defaultsize = this->module->getLLVMModule()->getDataLayout().getPointerSize(0);
	if (isa<IntType>(elemType)) {
		return cast<IntType>(elemType)->getSize() / 8;
	}

	else if (isa<FloatType>(elemType)) {
		return cast<FloatType>(elemType)->getSize() / 8;
	}
	//todo::fixx64 binary
	else if (isa<PointerType>(elemType)) {
		return defaultsize;
	}
	else if (isa<ArrayType>(elemType)) {
		uint64_t field_offset = 0;
		ShPtr<ArrayType> arrayType = cast<ArrayType>(elemType);
		ShPtr<Type>	elemType1 = arrayType->getContainedType();
		for (uint64_t dim : arrayType->getDimensions())
		{
			uint64_t elesize = defaultsize;
			if (isa<IntType>(elemType1)) {
				elesize = cast<IntType>(elemType1)->getSize() / 8;
				
			}

			if (isa<FloatType>(elemType1)) {
				elesize= cast<FloatType>(elemType1)->getSize() / 8;
			}
			//todo::fixx64 binary
			if (isa<PointerType>(elemType1)) {
				
			}
			field_offset += elesize*dim;
		}
		return field_offset;
	}
	return defaultsize;
...
bool HLLWriter::emitFileHeader() {
	out->commentLine("");
	out->commentLine("This file was generated by the GuoliSec Decompiler");
	out->commentLine("Website: https://www.guolisec.com");
	out->commentLine("");
