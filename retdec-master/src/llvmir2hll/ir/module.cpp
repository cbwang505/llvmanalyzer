...
void Module::addGlobal2VariablePair(llvm::GlobalVariable* g, ShPtr<Variable> v)
{
	auto& kv = Var2Global.find(v);
	if(kv== Var2Global.end())
	{
		Var2Global.emplace( v,g);
	}
}
bool Module::isSkipGlobalVariable(llvm::GlobalVariable* g)
{
	if (g->hasName())
	{
		llvm::StringRef varname = g->getName();
		if (varname.contains("Class_vtable"))
		{
			return true;
		}
	}
...
bool Module::isSkipVariable(ShPtr<Variable> v)
{
	for(auto& kv : Var2Global)
	{
		if(kv.first==v)
		{
			return isSkipGlobalVariable(kv.second);
		}
	}
	 return false;
}
