...
void LLVMIR2BIRConverter::convertAndAddGlobalVariables() {
	...
	for (llvm::GlobalVariable& globVar : llvmModule->globals()) {
		llvm::GlobalVariable* gv = &globVar;
		if (shouldBeConvertedAndAdded(globVar) | resModule->isSkipGlobalVariable(gv)) {
			ShPtr<Variable> variable = convertGlobalVariable(globVar);
			ShPtr<Expression> initializer = convertGlobalVariableInitializer(globVar);
			resModule->addGlobalVar(variable, initializer);		
			resModule->addGlobal2VariablePair(gv, variable);
