...
bool LlvmIr2Hll::initialize(llvm::Module &m)
{
	...
	if (outStringStream)
	{
		...
		idaStructWriter=llvmir2hll::IDAStructWriterFactory::getInstance().createObject<
			raw_string_ostream &>(TargetIDAHLL, *outStringStream, globalConfig->parameters.getOutputFormat());
...
void LlvmIr2Hll::emitTargetHLLCode()
{
	...
	if(idaStructWriter)
	{
		idaStructWriter->emitTargetCode(resModule);
	}
