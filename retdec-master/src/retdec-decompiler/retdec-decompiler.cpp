...
void ProgramOptions::loadOption(std::list<std::string>::iterator& i)
{
	...
	else if (isParam(i, "-o", "--output"))
	{
		...
		params.setOutputBinFile(out + ".bin");
...
void ProgramOptions::afterLoad()
{
	...
	if(params.getOutputBinFile().empty())
		params.setOutputBinFile(in + ".bin");
 ...
 * rapidjson.
 */
void setLogsFrom1(const retdec::config::Parameters& params)
{
...
{
	setLogsFrom1(config.parameters);
	...
	try
	{
		...
		if (config.parameters.isTimeout())
		{
			...
			else
			{
				...
				Log::error() << "timeout after: " <<std::to_string( config.parameters.getTimeout())
						<< " seconds" << std::endl;
				ret = EXIT_TIMEOUT;
			}
		}
