...
bool decompile_ida(retdec::config::Config& config, std::tuple<std::unique_ptr<retdec::utils::io::Logger>&, std::unique_ptr<retdec::utils::io::Logger>&, std::unique_ptr<retdec::utils::io::Logger>&> tp, uint64_t segaddr, std::string* outString, std::vector<std::uint8_t>* outBuff)
{
	//setLogsFrom(config.parameters);

	Log::set(Log::Type::Info, std::move(std::get<0>(tp)));
	Log::set(Log::Type::Debug, std::move(std::get<1>(tp)));
	Log::set(Log::Type::Error, std::move(std::get<2>(tp)));
	Log::phase("Initialization");
	auto& passRegistry = initializeLlvmPasses();
	...
	for (auto& p : config.parameters.llvmPasses)
	{
		if (auto* info = passRegistry.getPassInfo(p))
		{
			...
			if(segaddr!=0&&outBuff&&info->getTypeInfo() == &bin2llvmir::BinWriter::ID)
			{
				auto* p = static_cast<bin2llvmir::BinWriter*>(pass);
				p->setRemapSeg(segaddr);
				p->setOutputBuff(outBuff);
			}
		}
		else
		{
